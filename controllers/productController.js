  ...
  try {
    ...
    if (!validation.isValid) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid product ID format. Must be a valid MongoDB ObjectId or numeric ID.",
    ...
    console.log(
      `Product found: ${product.name}. Total views: ${product.views}`,
    );
...
const addProduct = async (req, res) => {
  try {
    ...
    const meta_title =
      req.body.meta_title ||
      `${req.body.name} - ${req.body.category} | Your Store`;
...
const updateProduct = async (req, res) => {
    console.log("req is hitting" , req.body)
  try {
    const { id } = req.body;

    const validation = validateProductId(parseInt(id));
    ...
    if (req.body.new_price !== undefined)
      updateData.new_price = req.body.new_price;
    if (req.body.old_price !== undefined)
      updateData.old_price = req.body.old_price;
    if (req.body.stock_quantity !== undefined)
      updateData.stock_quantity = req.body.stock_quantity;
...
const getProductAnalytics = async (req, res) => {
  try {
    ...
    const analytics = {
      ...
      conversion_rate:
        product.views > 0
          ? ((product.sales_count / product.views) * 100).toFixed(2)
          : 0,
      stock_status: {
        current_stock: product.stock_quantity || 0,
        low_stock_threshold: product.low_stock_threshold || 10,
...
const incrementSalesCountOfProduct = async (req, res) => {
  try {
    const productId = parseInt(req.params.id);
...
const bulkStatusOperations = async (req, res) => {
  try {
...
const generateSampleDataForProductsAndSales =  async (req, res) => {
    try {
...
const productInventoryManagement = async (req, res) => {
    try {
...
const productSeoSitemap = async (req, res) => {
    try {
        const products = await Product.find(
