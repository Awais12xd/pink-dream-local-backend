...
const getCart = async (req, res) => {
  try {
    ...
    if (validItems.length !== cart.items.length) {
      ...
      await cart.save();
      console.log(
        `Removed ${removedItems.length} unavailable items from cart for user ${userId}`,
      );
    }
    ...
    console.log(
      `Cart loaded: ${cartItemsWithDetails.length} unique items, ${totalItems} total items`,
    );
    ...
    if (localCartItems && localCartItems.length > 0) {
      for (const localItem of localCartItems) {
        try {
          ...
          if (
            product.stock_quantity !== undefined &&
            product.stock_quantity < localItem.quantity
          ) {
          ...
          console.error(
            `Error processing session item ${localItem.id}:`,
            itemError,
          );
          syncResults.failedItems.push({
            item: localItem,
